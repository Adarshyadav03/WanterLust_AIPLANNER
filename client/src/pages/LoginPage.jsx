...
import { Compass, Mail, Lock, LogIn, ShieldAlert, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-slate-900 border border-slate-800 p-8 sm:p-10 rounded-3xl shadow-2xl">
        <div className="text-center space-y-3">
          ...
          <h2 className="text-2xl font-black text-white">Welcome back to WanderLust</h2>
          <p className="text-xs text-slate-400">Sign in to access your AI trip itineraries and group chats</p>
        </div>
        ...
        {(error || oauthErr) && (
          <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-3.5 rounded-xl text-xs flex items-center gap-2">
            ...
            <span>{error || (oauthErr === 'google_oauth_failed' ? 'Google authentication failed. Please try again.' : 'Authentication failed. Please try again.')}</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <a href="#forgot" onClick={(e) => { e.preventDefault(); alert('Reset password link sent to your email.'); }} className="text-xs text-emerald-400 hover:underline">Forgot password?</a>
            </div>
            <div className="relative">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-extrabold rounded-xl text-sm transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2"
          >
        ...
        <div className="space-y-3">
          <div className="relative flex items-center justify-center">
            ...
            <span className="bg-slate-900 px-3 text-[10px] text-slate-500 font-bold uppercase tracking-wider absolute">Or continue with</span>
          </div>
